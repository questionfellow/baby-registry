...
export const GiftCard: React.FC<GiftCardProps> = ({ gift, onMarkAsGifted }) => {
    ...
    return (
        ...
        <div className={`glass-card overflow-hidden h-full flex flex-col ${isFullyGifted ? 'opacity-80' : ''}`}>
            ...
            {showBuyNowPopup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in" onClick={() => setShowBuyNowPopup(false)}>
                    <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
                        <button
